...
%DIF DEL diffbase.tex                     Fri Jun 18 13:25:01 2021
%DIF ADD policar2020-tsne-embedding.tex   Fri Jun 18 13:23:40 2021
...
\cite{wold1987principal} }\hskip0pt%DIFAUXCMD
}\DIFaddend and multi-dimensional
scaling~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{distill,umap_single_cell}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cox2008multidimensional}}\hskip0pt%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Our contributions are two-fold: 
}\begin{enumerate}
  \item \DIFadd{We introduce a theoretically-grounded extension of the t-SNE
  ...
  \item \DIFadd{We show that the proposed t-SNE extensions can mitigate batch effects
...
\cite{Tung2017,Hicks2018}}\hskip0pt%DIFAUXCMD
. When performing downstream
...
\item \DIFadd{mutual nearest neighbor-based approaches aim to identify matching
 populations of cells across the data sets, using them to either find and correct the data sets~\mbox{%DIFAUXCMD
\cite{Haghverdi2018} }\hskip0pt%DIFAUXCMD
or directly construct a
 batch-corrected k-nearest neighbor graph used in downstream analyses~\mbox{%DIFAUXCMD
\cite{Park2018}}\hskip0pt%DIFAUXCMD
.
}\item \DIFadd{embedding multiple data sets into a joint lower-dimensional space, where
 batch effects are removed. Some of these approaches opt for linear
 dimensionality-reduction methods such as PCA~\mbox{%DIFAUXCMD
\cite{Korsunsky2019} }\hskip0pt%DIFAUXCMD
or
 MultiCCA~\mbox{%DIFAUXCMD
\cite{Butler2018}}\hskip0pt%DIFAUXCMD
, while others employ non-linear techniques from
 deep learning~\mbox{%DIFAUXCMD
\cite{Li2020,Lopez2018}}\hskip0pt%DIFAUXCMD
. Still, other approaches use a
 combination of the two~\mbox{%DIFAUXCMD
\cite{Stuart2019,Hie2019}}\hskip0pt%DIFAUXCMD
. Note that these approaches bear similarity with transfer learning~\mbox{%DIFAUXCMD
\cite{weiss2016survey}}\hskip0pt%DIFAUXCMD
, which has also been used in domain adaptation~\mbox{%DIFAUXCMD
\cite{liu2019jointly}}\hskip0pt%DIFAUXCMD
.
}\end{enumerate}

\DIFadd{Besides computational techniques, approaches for the removal of batch effects
...
\cite{Kiselev2018}}\hskip0pt%DIFAUXCMD
...
\DIFadd{Our approach to batch effect removal falls into the second category, as we
...
before~\cite{Maaten2008,Becht2019}. Below, we list the data sets \DIFdelbegin \DIFdel{,
...
\cite{Sekhar2016}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Shekhar2016}}\hskip0pt%DIFAUXCMD
...
that handling batch effects were central to our \DIFdelbegin \DIFdel{endeavour }\DIFdelend \DIFaddbegin \DIFadd{endeavor }\DIFaddend and that the
...
\DIFaddbegin \subsection{\DIFadd{Construction of a reference embedding}}

\DIFaddend We use a number of additional, recently proposed modifications to enhance the
...
\DIFdelbegin \DIFdel{initializations}\DIFdelend \DIFaddbegin \DIFadd{initialization}\DIFaddend , this produces even more meaningful visualizations where
...
\begin{figure}[htbp]
  ...
  may lead to under-clustering of the more specialized cell types. \DIFaddbeginFL \DIFaddFL{In our example, the neuronal subclusters are more clearly defined in (b).}\DIFaddendFL }
...
\begin{figure}[htbp]
  ...
  \caption{Comparison of different initialization schemes for positioning new
   ...
   Campbell \etal~\cite{Campbell2017} using the median \DIFdelbeginFL \DIFdelFL{initizaliation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{initialization }\DIFaddendFL scheme
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \subsection{\DIFadd{On requirement of a complete reference set}}

\DIFadd{Our }\DIFaddend approach assumes that all cell types from the secondary data set are
present in the reference. \DIFaddbegin \DIFadd{Intuitively, using t-SNE in such a way is
conceptually similar to classification via $k$-nearest neighbor classifiers
and is similarly limited. }\DIFaddend The method may fail to reveal \DIFdelbegin \DIFdel{novel }\DIFdelend \DIFaddbegin \DIFadd{unseen }\DIFaddend cell types in
the secondary data set, \DIFdelbegin \DIFdel{possibly }\DIFdelend \DIFaddbegin \DIFadd{likely }\DIFaddend positioning them arbitrarily close to
unrelated clusters. \DIFdelbegin \DIFdel{Procedures such as scmap were recently proposed to cope
with such cases and identify the cells whose type is
new and not included in
the
reference}\DIFdelend \DIFaddbegin \DIFadd{In some instances, unknown cell types may be sufficiently
...
\etal~\mbox{%DIFAUXCMD
\cite{Hrvatin2018}}\hskip0pt%DIFAUXCMD
. We then select only non-neuronal cells from Campbell
\etal~\mbox{%DIFAUXCMD
\cite{Campbell2017} }\hskip0pt%DIFAUXCMD
and add them to the reference embedding in Fig.~\ref{fig:incomplete_reference}.b. The non-neuronal cells from Hrvatin \etal are
...
unknown cell types}\DIFaddend ~\cite{Kiselev2018}.


\DIFaddbegin \begin{figure}[htb]
  \includegraphics[width=\textwidth]{incomplete_reference-brain.pdf}
  \caption{\DIFaddFL{A reference embedding must contain all the cell types in the secondary embedding to produce reliable results. }{\bf \DIFaddFL{(a)}} \DIFaddFL{We construct a reference embedding containing only neuronal cells from Hrvatin \etal~\mbox{%DIFAUXCMD
\cite{Hrvatin2018}}\hskip0pt%DIFAUXCMD
. }{\bf \DIFaddFL{(b)}} \DIFaddFL{We select only non-neuronal cells from Campbell \etal~\mbox{%DIFAUXCMD
\cite{Campbell2017} }\hskip0pt%DIFAUXCMD
...
\DIFdelbegin \DIFdel{visualisation }\DIFdelend \DIFaddbegin \DIFadd{visualization }\DIFaddend plane is not only indicative but also an accurate instrument for
...
scMap-Cluster~\cite{Kiselev2018}\DIFdelbegin \DIFdel{, a $k$-nearest neighbor based method, tailored explicitly to scRNA-seq data. }\DIFdelend \DIFaddbegin \DIFadd{. For }\DIFaddend scMap-Cluster\DIFdelbegin \DIFdel{uses three correlation-based distance
...
\DIFdelbegin \DIFdel{Similarily }\DIFdelend \DIFaddbegin \DIFadd{Similarly }\DIFaddend to scMap-Cluster, we use the cosine distance to find the 5 nearest
...
\cite{opentsne}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Policar2019}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend Single-cell data sets suffer from high levels of technical noise and low
capture efficiency, resulting in sparse \DIFaddbegin \DIFadd{and noisy }\DIFaddend expression matrices~\cite{umi}. \DIFaddbegin \DIFadd{A common occurrence in these data sets is ``dropout'', where an
expressed gene is not measured, and its corresponding matrix entry is set to
zero. }\DIFaddend To address this problem, we use a specialized feature-selection method,
which exploits the mean-dropout relationship of expression counts as recently
proposed by Kobak and Berens~\cite{Kobak2019}. \DIFdelbegin \DIFdel{Here, genes with higher
than expected dropout rateare regarded as potential markers for cell
subpopulations and are retained in the data}\DIFdelend \DIFaddbegin \DIFadd{In this context, we will refer
...
subsequent analysis where we wish to discriminate between cell types}\DIFaddend .
